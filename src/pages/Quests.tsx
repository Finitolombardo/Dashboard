...
export default function Quests() {
  ...
  const filtered = mockQuests.filter(q => {
  ...
  return (
    <div className="h-screen flex flex-col">
      <PageHeader
        title="Quests"
        subtitle={`${mockQuests.length} Quests insgesamt`}
        actions={
            ...
            >
              ...
              {tab.value !== 'all' && (
                <span className="ml-1.5 text-2xs opacity-60">
                  {mockQuests.filter(q => q.status === tab.value).length}
                </span>
              )}
            </button>
          ))}
          ...
          {filtered.map(quest => (
            ...
            >
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <AgentChip agent={getAgentById(quest.agent_id)} />
                <div className="w-20">
